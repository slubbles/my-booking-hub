...
const ResumePage = () => {
  useSEO({ title: "Experience", description: "Experience of Idderf Salem - Full Stack Developer with production experience in React, TypeScript, and Node.js.", path: "/experience" });
      ...
      <div className="py-24 md:py-32">
        <div className="container mx-auto px-6 max-w-2xl">
            ...
            <div className="flex items-start justify-between gap-4 mb-10">
              <div>
                ...
                <div className="flex items-center gap-4 mt-3 text-[13px] text-muted-foreground/70 flex-wrap">
                  <span className="flex items-center gap-1.5"><Globe size={13} /> idderfsalem.dev</span>
                  <span className="flex items-center gap-1.5"><Mail size={13} /> Available for hire</span>
          ...
          <ScrollReveal delay={0.05}>
            <div className="mb-12">
              <div className="flex items-center gap-2.5 mb-5">
                ...
                <h2 className="text-[15px] font-semibold uppercase tracking-[0.15em] text-foreground">Work Experience</h2>
              </div>
              {experience.map((exp, i) => (
                <div key={i} className="bg-card border border-border/60 rounded-2xl p-6 premium-shadow">
                  <div className="flex items-center justify-between flex-wrap gap-2 mb-1">
                    <h3 className="text-[16px] font-semibold text-foreground">{exp.role}</h3>
                    {exp.active && (
                      <span className="px-2.5 py-0.5 text-[11px] font-medium rounded-full bg-primary/[0.08] text-primary tracking-wide">Active</span>
                    )}
                  </div>
                  <p className="text-[13px] text-muted-foreground/70 mb-4">{exp.company} · {exp.period}</p>
                  <ul className="space-y-2.5">
                    {exp.points.map((point, j) => (
                      <li key={j} className="flex gap-3 items-start">
                        <span className="text-[11px] text-primary/50 font-mono mt-0.5 flex-shrink-0 w-4 text-right">{String(j + 1).padStart(2, "0")}</span>
                        <p className="text-[14px] text-muted-foreground leading-[1.75]">{point}</p>
                      </li>
                    ))}
                  </ul>
                </div>
          ...
          <ScrollReveal delay={0.07}>
            <div className="mb-12">
              <div className="flex items-center gap-2.5 mb-5">
                ...
                <h2 className="text-[15px] font-semibold uppercase tracking-[0.15em] text-foreground">Key Projects</h2>
              </div>
              <div className="space-y-4">
                {projects.map((project) => (
                  <div key={project.name} className="bg-card border border-border/60 rounded-2xl p-5 premium-shadow hover:premium-shadow-hover transition-all duration-500">
                    ...
                    <p className="text-[13px] text-muted-foreground leading-[1.7]">{project.desc}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          <ScrollReveal delay={0.09}>
            <div className="mb-12">
              <div className="flex items-center gap-2.5 mb-5">
                ...
                <h2 className="text-[15px] font-semibold uppercase tracking-[0.15em] text-foreground">Technical Skills</h2>
              </div>
                ...
                {Object.entries(skills).map(([category, items]) => (
                  <div key={category} className="bg-card border border-border/60 rounded-2xl p-5 premium-shadow">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {items.map((skill) => (
                        <span key={skill} className="px-2.5 py-1 text-[12px] rounded-full bg-secondary text-muted-foreground/80 font-medium">
                          {skill}
                        </span>
                      ))}
                    </div>
          ...
          <ScrollReveal delay={0.11}>
            <div>
              <div className="flex items-center gap-2.5 mb-5">
                ...
                <h2 className="text-[15px] font-semibold uppercase tracking-[0.15em] text-foreground">Certifications & Training</h2>
              </div>
              <div className="grid sm:grid-cols-3 gap-4">
                {education.map((edu) => (
                  <div key={edu.title} className="bg-card border border-border/60 rounded-2xl p-5 premium-shadow hover:premium-shadow-hover transition-all duration-500">
                    <p className="text-[11px] uppercase tracking-[0.25em] text-muted-foreground/60 mb-1">{edu.type}</p>
                    <h3 className="text-[14px] font-semibold text-foreground">{edu.title}</h3>
                  </div>
                ))}
              </div>
            </div>
          ...
          <ScrollReveal delay={0.13}>
            <p className="text-[12px] text-muted-foreground/50 text-center mt-14">
